...
class DatabaseManager:
    ...
    # API Call Metrics CRUD Operations

    @critical_retry_operation
    async def create_api_call_metrics(
        self,
        call_id: str, # Was message_id
        model_id: str,
        prompt_tokens: int,
        ...
        """
        Create a new API call metrics record.

        Args:
            call_id: Unique call identifier (generation ID or UUID)
            ...
            message_id: Optional logical message ID
            agent_type: Optional agent type

        Returns:
            Created ApiCallMetrics instance
        """
        async with self.async_session_maker() as session:
            try:
                ...
                logger.info(f"Created API call metrics: {call_id}")
                return metrics
            except Exception as e:
                await session.rollback()
                logger.error(f"Failed to create API call metrics: {e}")
                raise

    async def get_api_call_metrics(self, call_id: str) -> Optional[ApiCallMetrics]:
        """Get API call metrics by ID."""
        async with self.async_session_maker() as session:
            result = await session.execute(
                select(ApiCallMetrics).where(ApiCallMetrics.call_id == call_id)
            )
            return result.scalar_one_or_none()

    async def update_api_call_actual_cost(self, call_id: str, actual_cost: float):
        """Update actual cost for a call after querying generation endpoint."""
        async with self.async_session_maker() as session:
            try:
                result = await session.execute(
                    select(ApiCallMetrics).where(ApiCallMetrics.call_id == call_id)
                ...
                if metrics:
                    ...
                    await session.commit()
                    logger.info(f"Updated actual cost for call {call_id}: ${actual_cost}")
                else:
                    logger.warning(f"API call metrics not found: {call_id}")
            except Exception as e:
                await session.rollback()
    ...
    async def update_session_metrics(
        ...
        """Update session metrics by adding API call metrics."""
        async with self.async_session_maker() as session:
            try:
                result = await session.execute(
                    select(SessionMetrics).where(SessionMetrics.session_id == session_id)
                ...
                metrics.call_count += 1 # Updated field name
    ...
    async def get_session_api_calls(self, session_id: str) -> List[ApiCallMetrics]:
        """Get all API call metrics for a session."""
        async with self.async_session_maker() as session:
            result = await session.execute(
                select(ApiCallMetrics)
                .where(ApiCallMetrics.session_id == session_id)
                .order_by(ApiCallMetrics.created_at)
            )
    ...
    async def update_project_metrics(
        ...
        is_new_call: bool = True # Updated parameter name
    ):
