...
impl KnowledgeManager {
    ...
    async fn populate_plugin_kb(&self) -> Result<()> {
        ...
        let commands = vec![
            ("create_scene", r#"Create a new scene in Godot.
...
Use case: Starting a new scene, creating UI menus, creating game levels"#),
            ...
            ("create_node", r#"Create a node in the current scene.
...
Use case: Adding nodes to scene hierarchy, building game objects"#),
            ...
            ("modify_node", r#"Modify properties of an existing node.
...
Use case: Changing node properties, positioning, scaling, configuring"#),
            ...
            ("attach_script", r#"Attach a GDScript to a node.
...
Use case: Adding behavior to nodes, implementing game logic"#),
            ...
            ("get_scene_info", r#"Get information about the current scene.
...
Use case: Understanding current scene structure, debugging, planning modifications"#),
            ...
            ("search_nodes_by_type", r#"Search for nodes by their type.
...
Use case: Finding all nodes of a specific type, batch operations"#),
            ...
            ("play", r#"Run the game in Godot editor.
...
Use case: Testing the game, running scenes"#),
        ...
        for (id, content) in commands {
            ...
            self.plugin_kb.add_document(id.to_string(), content.to_string(), metadata).await?;
        }
    ...
    async fn populate_docs_kb(&self) -> Result<()> {
        ...
        let docs = vec![
            ("node2d_overview", r#"Node2D is the base class for all 2D nodes in Godot.
...
Use Node2D as the root for 2D game scenes."#),
            ...
            ("characterbody2d", r#"CharacterBody2D is used for player-controlled or AI-controlled characters in 2D games.
...
Example usage: Player characters, enemies, NPCs with custom movement logic."#),
            ...
            ("control_nodes", r#"Control is the base class for all UI nodes in Godot.
...
Container nodes automatically arrange their children."#),
            ...
            ("sprite2d", r#"Sprite2D displays a 2D texture/image.
...
Can be animated by changing the texture or using region_rect for sprite sheets."#),
            ...
            ("signals", r#"Signals are Godot's implementation of the observer pattern for event handling.
...
Common built-in signals: pressed (Button), body_entered (Area2D), timeout (Timer)"#),
            ...
            ("gdscript_basics", r#"GDScript is Godot's built-in scripting language.
Key concepts:
