...
impl TutorialProcessor {
    pub fn new(api_key: &str) -> Self {
        Self { api_key: api_key.to_string(), client: Client::new() }
    }

    pub async fn fetch_godot_tutorials(&self, topic: &str, godot_version: Option<&str>) -> Result<String> {
        if std::env::var("GODOTY_OFFLINE").ok().as_deref() == Some("1") {
            return Ok(format!("[Tutorials Offline] topic='{}' version='{}'", topic, godot_version.unwrap_or("unknown")));
        }

        #[derive(Serialize)]
        struct Msg { role: String, content: String }
        #[derive(Serialize)]
        struct Req { model: String, messages: Vec<Msg>, temperature: f32, max_tokens: i32 }
        #[derive(Deserialize)]
        ...
        let req = Req {
            ...
            messages: vec![
                Msg { role: "system".into(), content: "You are a research assistant that finds Godot tutorials.".into() },
        ...
        match resp {
            Ok(rsp) if rsp.status().is_success() => {
                ...
                if let Ok(parsed) = serde_json::from_str::<Resp>(&body) {
                    if let Some(choice) = parsed.choices.first() { return Ok(choice.message.content.clone()); }
                }
